...
numGrains = 70; %number of grains to pack into the grid
...
totalTime=100; %total time to run the simulation
maxNodeVelocity=1.5; %maximum node velocity in a given timestep

%const.mobility=2;
const.mobility=0.00005;
const.dt = dt;
...
writeMovie=0; %1==write frames to an avi file, 0==don't generate movie
movieTitle="herring_junction3";
...
for t = dt:dt:totalTime
    ...
    tic
    [nodeBelong,nodeLoc,nodeConnect,segRadius,nodeVel]=refineMesh(nodeBelong,nodeLoc,nodeConnect,segRadius,nodeVel,minRemeshDistance/8,gridSize); %merge nodes that need to be merged
    [nodeBelong,nodeLoc,nodeConnect,segRadius,nodeVel] = requireTripleJunctions(nodeBelong,nodeLoc,nodeConnect,segRadius,nodeVel,minRemeshDistance);
    ...
    [posUpdates,nodeVel] = forwardEuler(nodeBelong,grainMat,nodeConnect,nodeLoc,nodeVel,misorientMat,const); %forward euler method for position integration
    ...
    if plotMicrostructure
        plotGrains(grid,nodeBelong,nodeLoc,nodeConnect,grainMat,segRadius,C); %plot the grains to axis
    end
